 ...
 */
//class FacebookGrant implements \League\OAuth2\Server\Grant\GrantTypeInterface {
class FacebookGrant extends \League\OAuth2\Server\Grant\AbstractGrant {
     ...
     * @var string
     */
    protected $identifier = 'facebook_access_token';
    ...
    /**
     * Callback to authenticate a user's facebook auth token
     * @var function
     */
    ...
    /**
     * Set the callback to verify a user
    ...
    public function completeFlow($inputParams = null)
    {
        ...
        $authParams = $this->authServer->getParam(array('client_id', 'client_secret', 'facebook_access_token'), 'post', $inputParams);
        ...
        if (is_null($authParams['facebook_access_token'])) {
            throw new Exception\ClientException(sprintf($this->authServer->getExceptionMessage('invalid_request'), 'facebook_access_token'), 0);
        }
 
        // Check token
        $userId = call_user_func($this->getVerifyCredentialsCallback(), $authParams['facebook_access_token']);
